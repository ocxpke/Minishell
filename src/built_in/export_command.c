...
static int	if_exists_replace(t_linked_env *env_list, char **key_value,
		int *ret)
{
	if (!ft_strncmp(env_list->key, key_value[0], ft_max_len_str(env_list->key,
				key_value[0])))
	{
		free(env_list->value);
		if (key_value[1])
...
void	export_cmd(t_shell_data *shell_data, int *ret)
{
	...
	*ret = 1;
	...
	while (shell_data->tokens[i])
	{
		...
		if (key_value && !find_and_replace_env(&(shell_data->shell_envi),
				key_value))
		{
